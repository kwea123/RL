...
class DDQNAgent:
    ...
    def set_model(self, model):
        ...
        self.model = model
        self.target_weights = np.asarray(model.get_weights()) # true weights
        self.model_set = True
    ...
    def error(self, state, action, reward, next_state):
        amax = np.argmax(self.model.predict(next_state)[0]) # use the new network to predict action
        current_weights = self.model.get_weights()
        self.model.set_weights(self.target_weights)
        target = (reward + self.gamma *
                 self.model.predict(next_state)[0][amax]) # use the old network to evaluate value
        self.model.set_weights(current_weights)
        target_f = self.model.predict(state) # get the current estimate
        return np.abs(target - target_f[0][action])
    ...
    def replay(self, batch_size):
        ...
        states, target_fs = [], []
        for idx, (state, action, reward, next_state) in minibatch:
            amax = np.argmax(self.model.predict(next_state)[0]) # use the new network to predict action
            current_weights = self.model.get_weights()
            self.model.set_weights(self.target_weights)
            target = (reward + self.gamma *
            ...
            if self.priority_alpha > 0: # prioritised, update
                self.memory.update(idx, self.error(state, action, reward, next_state))
